(function () {
  ...
  var common = {
    ...
    '招募大厅': ['教师端招募业务预留','后续承载招募活动、任务、教师报名和进度查询。','本期范围|建议结构|入口规则|待确认','本期暂不实现，当前仅保留入口与空白预留页，原型不做改动。;活动展示对象、条件、截止时间和状态，报名后展示审核进度。;从授课中心进入并返回授课中心。;招募主体、报名资料、审核角色和推广素材联动。'],
    '授课中心': ['教师端授课工作台','展示资料状态、工作入口、教学数据、排课日历和已排课程。','本期范围|资料状态|工作入口|授课地点|重点测试','招募大厅、授课管理本期暂不实现，两个入口仅作占位保留，原型不做改动。;此页仅显示“已驳回”；原因在教员资料查看，点击编辑资料后修改。;招募大厅和授课管理位于教师信息与数据统计之间；日期切换课程。;线上：西安小寨交付中心；线下：西安小寨校区；上门：上门。;模拟驳回、资料跳转、月份和无课日期。'],
    '授课管理': ['教师授课能力与设置预留','后续维护可授课程、时间、授课方式或接单设置。','本期范围|建议范围|入口规则|待确认','本期暂不实现，当前仅保留入口与空白预留页，原型不做改动。;维护年级、科目、教材、方式和时间段，设置影响匹配与排课。;从授课中心进入并返回授课中心。;停课权限、接单范围和变更审核规则。'],
