(function () {
  ...
  var title = document.title.replace(/\s+v\d+(?:\.\d+)?$/, '').replace(/^(教师端|家长端)\s*[·•]\s*/, '').trim();
  ...
  var common = {
    ...
    '业务中心': ['合伙人业务工作台','集中展示课时、会员、课程销售和常用业务入口。','信息结构|交互规则|重点测试','只展示当前账号有权限的业务指标，没有内容的字段不展示。;数据卡片进入明细，业务入口进入办理流程。;身份权限、空数据、入口跳转和返回状态。'],
    '个人中心': ['统一身份与服务入口','聚合身份、会员、教师端/家长端、创客中心、推广中心和个人设置。','身份展示|入口规则|重点测试','只展示账号真实拥有的身份与会员状态。;授课中心面向教师，学习中心面向家长，推广中心进入素材广场。;多身份组合、各入口和退出登录确认。'],
