(function () {
  ...
  var common = {
    '上课记录': ['家长端上课记录与约课统一列表','融合原「我的约课」与上课记录：按全部课程、待上课、待确认、待评价、已完成、已取消查看，支持取消改约与评价入口，并可按月份、课程、授课方式筛选。','状态口径|课时与操作|重点测试','状态由约课流程产生：待确认→待上课（老师确认并冻结课时）→待评价（签到成功、扣减课时）→已完成（老师提交反馈）；已取消含家长取消与老师拒绝。;待确认可取消约课、待上课可取消改约，取消需选原因且冻结课时立即释放，已取消可重新约课；待评价进入评价，已完成展示课程反馈与课后作业并进入课程详情。;六标签切换、月份/课程/授课方式组合筛选、取消释放课时、评价与重新约课入口和空状态。'],
    ...
    '学习中心': ['家长端学习工作台','汇总学员资料、课时统计、课程计划与约课入口；「已上课时」可进入上课记录。','课时与约课|日历联动|入口规则|课程详情|重点测试','剩余可用课时已剔除冻结课时，1 课时 = 45 分钟；冻结在老师确认排课后按课程时长产生，签到回调成功后才扣减。;家长端只保留「去约课」入口，约课状态与历史记录统一在「上课记录」查看（原「我的约课」已合并）；课时统计的「已上课时」点击进入上课记录，本页不再重复展示记录列表。;日历支持按实际年月切换，约课日期显示圆点，蓝色为已确认、琥珀色为待确认，点击日期优先展示当日约课记录。;课程进入详情，待评价在基础信息区右侧显示“去评价”。;月份边界、无课日期、入口跳转和课时口径。'],
    ...
    '授课记录': ['教师端约课处理 + 授课记录','家长提交的约课与授课历史统一在这里处理：累计授课、待确认、待上课、待反馈、已完成，可按月份和授课方式筛选。','状态口径|约课处理|演示开关|重点测试','五个状态与家长端一一对应：待确认→待上课（老师确认并冻结家长课时）→待反馈（签到成功、已扣课时）→已完成（老师提交反馈），已取消（家长取消或老师拒绝）保留在「累计授课」。;待确认可确认接单或选原因拒绝，待上课可联系家长，待反馈进入课程反馈，已完成可查看课程；确认后按课程时长冻结家长课时（1 课时 = 45 分钟）。;左上角可切换演示老师、模拟第三方签到回调，仅用于原型演示，不代表真实签到；批量提交的约课逐节生成记录并标注“批量 x/y”。;五标签计数与待确认小圆点、月份/授课方式筛选、确认与拒绝、签到扣减、反馈回写和空状态。'],
    ...
    '约课': ['家长端确认老师并选择具体上课时间','先选课程并确认老师（默认推荐曾给该学员上过这门课的老师，首次约课系统自动匹配，均支持更换），再选择具体日期和时刻；支持每周 / 每天批量预约。','课程选择|老师确认|日期与时刻|批量预约|重点测试','学员默认带入；课程展示剩余可用课时、默认上课方式和消耗课时。;老师卡片展示推荐理由（历史老师 / 首次匹配）与空闲时间概览（工作日全天 / 周末全天 / 全部时间，未整天时列出具体时段）+ 未来 4 周可约时段数，点“更换”可在候选老师中切换。;日期只列出该老师已开放且还有空档的日子，时刻按每天 08:00 - 23:30、每 30 分钟一档（32 档）展示，已被约显示“已被约”、未开放显示“未开放”并置灰。;批量预约：每周同一天同一时间连约 4 次、每天同一时间连约 7 天；预览逐条标注可约与跳过原因，不可约的日期自动跳过。;课程切换、老师更换、批量预览、剩余课时不足和提交跳转。'],
    '可约时段': ['教师端维护四类时间与特殊日期','四类时间长期生效，特殊日期只覆盖某一天；家长端按此展示可约日期和时刻。','四类时间|特殊日期|与约课联动|重点测试','四类时间可多选：工作日白天（08:00 - 17:30，20 档）、工作日晚上（18:00 - 23:30，12 档）、周末白天、周末晚上，每 30 分钟一档；特殊日期可用「全天开放 / 当天不开放」快捷设置。;特殊日期优先于四类时间，可只开放某一天的个别时段，也可设为当天不开放，并能一键恢复为跟随四类时间。;已提交的约课不受开关影响；页面顶部不重复老师概览卡（教师身份与可约概览只在授课中心教师资料卡展示），页尾提示“特殊日期优先于四类时间”；特殊日期列表每条带「删除」直接恢复跟随四类时间，卡片右侧另有「特殊日期」总开关，关闭后所有特殊日期不生效（数据保留，可再开启），家长端只按四类时间展示；重点测试：老师切换、四类开关、单条删除、特殊日期保存与恢复、总开关联动家长端。'],
